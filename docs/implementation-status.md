...
| App Store Connect purchase catalog | Complete; ready for sandbox testing 2026-08-26 | Group `22336496`; monthly `6805458388` ($3.99), annual `6805458489` ($19.99), and lifetime `6805458712` ($44.99) are `READY_TO_SUBMIT`. Family Sharing, all 175 current territories, automatic new-territory availability, equalized pricing, version-scoped en-US metadata, review images, and one-week trials for both subscriptions are configured. Product validators report only the expected first-review attachment warnings. |
| 5.2a Restore Purchases | Complete | Settings row on `AppStore.sync()`, three outcomes kept distinct, always visible |
| 5.2 Paywall UI | Code complete; transaction walkthrough pending | Custom warm-paper monthly/annual/lifetime paywall, annual preselection, StoreKit trial eligibility, restore/legal/loading/retry/pending/cancelled states, Settings upgrade/active/manage controls, and automatic post-purchase continuation compile in Debug and Release. Live ASC products and localized prices were loaded in the simulator on 2026-08-26; purchase/restore outcomes still need sandbox-device verification. |
